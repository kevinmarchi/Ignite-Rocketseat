...
export async function getWeekPendingGoals() {
    ...
    const goalCompletionCounts = db.$with('goal_completion_counts').as(
        ...
        }).from(goalCompletions).where(and(gte(goalCompletions.createdAt, firstDayOfWeek),lte(goalCompletions.createdAt, lastDayOfWeek))).groupBy(goalCompletions.goalId)
    )
